...
- [ ] If these handoff documents have not merged to main, fetch `origin/docs/oauth-agent-demo-handoff`, then import only these two files into the new feature branch with `git restore --source=origin/docs/oauth-agent-demo-handoff -- docs/BEACON_OAUTH_DEMO_RUNBOOK.md docs/superpowers/plans/2026-09-19-beacon-oauth-agents-demo.md` and commit them. This keeps the executable handoff available after switching away from the documentation branch; skip the import when identical documents already exist.
