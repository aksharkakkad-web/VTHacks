...
Read next: [owner-by-owner updates and build order](../../superpowers/plans/2026-09-19-beacon-provider-network-pivot.md).
...
The consolidation carries forward the omission-preserving fix from [PR #17](https://github.com/aksharkakkad-web/VTHacks/pull/17), alongside Akshar's nullable ranked result. Provider normalization no longer invents zero transfers. Agent and decision tests verify this boundary; the legacy scoring limitation remains explicitly warned. Rishit's UI is included in the same checkout but still uses its frontend simulation.
