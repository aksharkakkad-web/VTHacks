...
2. Cancellation still sends fresh offers, an incremented `objectiveVersion`, and `excludedProviderIds`. Recheck quote expiry and current objective at confirmation/booking. A SQL recommendation is not provider identity verification, authorization or a booking.
