...
Mahin owns backend coordination and the Google Routes adapter. Akshar owns comparison of complete journeys, evidence, policy, constraints and ranking. Rishit owns the phone experience. No frontend changes are part of this backend follow-up.
...
Akshar should adapt `WalkingRoute` into the complete-journey evaluator, retaining the route ID and fetched time. Google geometry alone does not establish lighting, public access or known-closure clearance. The adapter deliberately marks closures `not_checked` and safety `not_assessed`. The existing mapped campus path remains its own source; it must not be drawn as a vehicle route. Do not substitute a Google walking duration into an already approved journey without reevaluation and fresh consent.
