...
This checkout has `Trip`, `CandidatePlan`, and `Recommendation` contracts but no implemented `/api/trips/**` routes, agent services, ANS client, or Databricks adapter. The interactive UI is a deterministic frontend simulation. Its timeline is demonstration evidence, not proof of sponsor integration.
