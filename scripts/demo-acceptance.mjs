...
try{
  if(!deterministic)await client.pair();
  ...
  if(deterministic)assert.equal(view.planning.explanationSource,'template');
  else if(!fixture)assert.equal(view.planning.explanationSource,'llm_grounded');
  ...
  const output={status:'passed',checkedAt:new Date().toISOString(),model:deterministic?'none':fixture?'fixture':planner.model,modelInference:deterministic?'none':fixture?'fixture_only':'codex_subscription',data:'synthetic_demo_scenario',rides:'simulated',notifications:'simulated',receipts};
