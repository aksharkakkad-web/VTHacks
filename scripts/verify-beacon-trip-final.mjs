...
async function seed(s){await page.goto(base+'/offline.html');await page.evaluate(s=>{localStorage.setItem('safecircle.profile.v1',JSON.stringify(s.profile));sessionStorage.setItem('beacon.demo-trip.v2',JSON.stringify({version:2,mode:'local-simulation',state:s}));},s);await page.goto(base+'/demo?walkthrough=1&transport=manual');await stage(s.stage);}
...
let s=snap('recommendation',{...defaultProfile,walkingPreference:'normal',avoidTransfers:false});s=act(s,{type:'SELECT_PLAN',planId:mode==='walk'?'walk-008':'transit-017',now:Date.now()});s=act(s,{type:'GO',now:Date.now()});if(mode==='transit')s=act(s,{type:'ADVANCE',now:Date.now()});s=act(s,{type:'CONFIRM_ARRIVAL'});await seed({...s,paused:true});await evidence(`${width}-${mode}-arrival-fixed`);const body=await page.locator('body').innerText();assert.match(body,/This plan is complete/);assert.match(body,/No active provider booking/);assert.doesNotMatch(body,/Provider access ended|Temporary provider access has ended/i);results.checks.push(`${width}: ${mode} arrival states local completion without claiming prior provider access`);
}
...
await tap('Request cancellation');await page.getByRole('dialog').waitFor();const dialog=await page.getByRole('dialog').innerText();assert(!dialog.includes('wait for the provider'));await evidence(`${width}-${mode}-cancel-sheet`);await page.getByRole('dialog').getByRole('button',{name:'Stop request',exact:true}).tap();await stage('home');await evidence(`${width}-${mode}-stopped`);assert.equal(await saved(),null,'stopped local plan is cleared instead of retaining provider authority');assert.equal(await page.locator('[data-stage="cancelling"]').count(),0);results.checks.push(`${width}: ${mode} cancellation immediately closes local plan without fictional provider cancellation`);
}
...
let longWalk=snap('recommendation',{...longProfile,walkingPreference:'normal'});longWalk=act(longWalk,{type:'SELECT_PLAN',planId:'walk-008',now:Date.now()});longWalk=act(longWalk,{type:'GO',now:Date.now()});await seed({...longWalk,paused:true});await evidence(`${width}-long-walking-destination`);assert.match(await page.locator('body').innerText(),/accessible east residential entrance/);await seed({...snap('waiting-initial',longProfile),paused:true});await tap('View trip details');await page.getByRole('dialog').waitFor();await evidence(`${width}-trip-details`);assert.match(await page.getByRole('dialog').innerText(),/Booking|booking/);results.checks.push(`${width}: long walking destination wraps; trip details and controls remain at least 44px with reduced motion`);
}
}
