/** Real mobile walkthrough + deterministic fixtures through the same production reducer. */
...
const out='docs/ui-research/beacon-integration/baseline';
...
try{
  ...
  await tap('Get me home');await state('discovering');await shot('06-discovery');await state('evaluating');await shot('07-comparison');await state('recommendation');await shot('08-recommendation');await p.getByRole('button',{name:'Confirm this plan',exact:true}).scrollIntoViewIfNeeded();await shot('08-offer-terms');await tap('Confirm this plan');
  for(const [s,id]of [['verifying-initial','09-identity'],['authorizing-initial','10-authorization'],['coordinating-initial','11-booking'],['accepted-initial','12-accepted'],['waiting-initial','12-pickup'],['arriving-initial','12-approaching'],['in-trip-initial','13-travelling']]){await state(s);await shot(id);}
  await tap('I’m home');await state('arrival');await shot('14-arrival');await tap('Finish');await state('home');checks.push('Main real-tap journey, including separate identity/access/payment/booking');
