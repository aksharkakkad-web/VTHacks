...
export function parseAtomicJourney(value:unknown):AtomicJourney {
 ...
 if(v.planning!==null){const p=object(v.planning);oneOf(p.phase,['understanding','gathering','evaluating','explaining','ready','needs_input','unavailable']);oneOf(p.worker,['online','offline','auth_required','rate_limited']);text(p.messageCode);if(p.explanation!==undefined)text(p.explanation);}
...
export function createPayload(profile:SavedProfile,context:TripContext={}){return {journeyContract:'beacon-journey-v1' as const,demoScenarioVariant:'baseline' as const,preferences:{maxBudget:profile.maxBudget,walkingPreference:profile.walkingPreference==='minimal'?'minimize':'normal',transferPreference:profile.avoidTransfers?'minimize':'normal'},temporary_context:{...(context.maxBudget!==undefined?{max_budget:context.maxBudget}:{}),...(context.walkingPreference?{minimize_walking:context.walkingPreference==='minimal'}:{}),has_been_drinking:context.note==='drinking',exhausted:context.note==='tired'}};}
export function normalizeJourney(snapshot:AtomicJourney,profile:SavedProfile,context:TripContext={}) {
