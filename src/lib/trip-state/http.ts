...
function session(request: Request, create = false) {
...
function secret(request: Request, expected: string | undefined) {
...
async function handle(fn: () => Promise<Response>) {
  try { return await fn(); }
    ...
    if (error instanceof Error && /^(Invalid|Expected|Contact)/.test(error.message)) return json({ error: { code: "INVALID_INPUT", message: "Check the trip request fields" } }, 400);
    return json({ error: { code: "SERVICE_UNAVAILABLE", message: "Trip service unavailable; please try again" } }, 503);
  }
}
export function createTrip(request: Request) { return handle(async () => { sameOrigin(request); const s = session(request, true); return json(await getRuntime().agent.create(s.owner, await body(request)), 201, s.cookie); }); }
export function getTrip(request: Request, id: string) { return handle(async () => json(await getRuntime().agent.read(id, session(request).owner))); }
export function tripAction(request: Request, id: string, action: string, demo = false) {
  return handle(async () => {
    sameOrigin(request);
    if (action === "events" && !demo) {
      const input = await body(request); secret(request, process.env.BEACON_PROVIDER_EVENT_TOKEN);
      return json(await getRuntime().agent.providerEvent(id, text(input.providerId, "provider id"), text(input.bookingId, "booking id"), text(input.event, "event")));
    }
    const allowed = demo ? ["cancel-provider", "expire-deadline"] : ["discover", "evaluate", "confirm", "verify", "request", "location", "arrive"];
    if (!allowed.includes(action)) throw new TripError("NOT_FOUND", "Operation not found", 404);
    const s = session(request); const input = await body(request);
