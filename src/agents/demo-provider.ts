...
import { advanceDemoRide, cancelDemoBooking, demoRideResult, initializeDemoRide, parseDemoRideEvent } from './demo-ride-simulation';
export { cancelDemoBooking } from './demo-ride-simulation';
...
export type DemoBooking = { result: ProviderTrip; sensitive?: TripRequest; fingerprint?: string; cancellationFeeMinor?: number; demoRide?: { etaUpdatedAt: number } };
...
export class DemoProvider {
  ...
  constructor(readonly descriptor: ProviderDescriptor, private readonly options: { token?: string; cancellationFeeMinor?: number; decline?: boolean; demoRideProgress?: boolean; now?: () => number } = {}) {}
  ...
  newBooking(id: string, prepared: ReturnType<DemoProvider["prepareBooking"]>): DemoBooking {
    ...
    const booking: DemoBooking = { result: { id, status: prepared.declined ? "declined" : "waiting", ...(payment ? { payment: prepared.declined ? simulatedCancellation(payment, 0) : payment } : {}) }, ...(prepared.declined ? {} : { sensitive: prepared.sensitive }), fingerprint: prepared.fingerprint, ...(prepared.cancellationFeeMinor === undefined ? {} : { cancellationFeeMinor: prepared.cancellationFeeMinor }) };
    if (this.demoRideProgressEnabled) initializeDemoRide(booking, this.descriptor.mode, fixtures[this.descriptor.mode][1] * 60, (this.options.now ?? Date.now)());
    return booking;
  }
  ...
  tripResult(booking: DemoBooking) { return demoRideResult(booking, (this.options.now ?? Date.now)()); }
  handle(method: string, path: string, data: unknown): unknown {
    ...
    if (method === "POST" && path === "/agent/request-trip") {
      ...
      if (known) {
        ...
        return this.tripResult(booking);
      }
    ...
    if (method === "GET" && path.startsWith("/agent/trip-status/")) return this.tripResult(this.get(decodeURIComponent(path.slice(19))));
    if (method === "GET" && path.startsWith("/agent/request-status/")) {
      ...
      return { trip: id ? this.tripResult(this.get(id)) : null };
    }
    if (method === 'POST' && path === '/agent/demo-advance') {
      if (!this.demoRideProgressEnabled) throw new TripError('DEMO_PROGRESS_DISABLED', 'Demo progression is not enabled', 404);
      const event = parseDemoRideEvent(data), booking = this.get(event.tripId);
      advanceDemoRide(booking, event, (this.options.now ?? Date.now)()); return this.tripResult(booking);
    }
    if (method === "POST" && path === "/agent/cancel-request") {
      ...
      cancelDemoBooking(booking, (this.options.now ?? Date.now)()); this.trips.set(id, booking); return this.tripResult(booking);
    }
    if (method === "POST" && path === "/agent/cancel-trip") {
      const trip = this.get(text(object(data).trip_id, "trip id"));
      cancelDemoBooking(trip, (this.options.now ?? Date.now)()); return this.tripResult(trip);
    }
...
export function providerHandler(provider: DemoProvider, token?: string) {
  ...
  return async (req: IncomingMessage, res: ServerResponse) => {
    ...
    try {
      ...
      if (path === '/agent/demo-advance' && !provider.signingToken) { res.writeHead(401); res.end(JSON.stringify({ error: 'Unauthorized' })); return; }
